...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(bd->g_hWnd != 0);

    const bool is_app_focused = (::GetForegroundWindow() == bd->g_hWnd);
    if (is_app_focused)
    {
        ...
        if (io.WantSetMousePos)
        {
            ...
            if (::ClientToScreen(bd->g_hWnd, &pos))
        ...
        {
            POINT pos;
            if (::GetCursorPos(&pos) && ::ScreenToClient(bd->g_hWnd, &pos))
